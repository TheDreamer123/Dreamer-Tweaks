...
public class AzaleaBlockMixin extends PlantBlock {
    ...
    public ActionResult onUse(BlockState state,World world,BlockPos pos,PlayerEntity player,Hand hand,BlockHitResult hit) {
        ...
        if(state.isOf(Blocks.FLOWERING_AZALEA)) {
            if(stack.isOf(Items.SHEARS)) {
                if (stack.isOf(Items.SHEARS)) {
                    if (!world.isClient) {
                        ...
                        world.setBlockState(pos,Blocks.AZALEA.getDefaultState(), 11);
        ...
        } else if(state.isOf(Blocks.AZALEA)) {
            if(stack.isOf(DreamerTweaks.AZALEA_BLOOM.asItem())) {
                if (!world.isClient) {
                    ...
                    world.setBlockState(pos,Blocks.FLOWERING_AZALEA.getDefaultState(), 11);
